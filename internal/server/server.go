...
func (s *Server) forwardAlert(logger *zap.Logger, alert *alertmanager.Alert) error {
	...
	// Add labels using template if configured, otherwise use default format
...
func (s *Server) renderLabelsTemplate(alert *alertmanager.Alert) ([]string, error) {
	// If no labels template is configured, use the default behavior
	...
	// If the template renders to an empty string, return no tags
	if renderedLabels == "" {
		return []string{}, nil
	}

	// Split the rendered output by comma and clean up each tag
	var tags []string
